...
We picked [cockroachdb](https://www.cockroachlabs.com/docs/secure-a-cluster.html).
...
- *vert_controller* will start `PlanController`, `DeploymentController, HorizontalAssemblyScaler, BillerController`
...
- *vert_scheduler* will scan for `assembly resource` to check its status to see if it needs to "SCHEDULE_" and will launch or manage using its `MultiCloud framework` in the appropriate region.

Every `MultiCloud framework` will load its own configuration. eg:

`/var/lib/megam/vertice/opennebula.conf`

The `PlanController` receives the request and will create plans *for future*.
...
/v3/plan_factory ?
/v3/plan/ubuntu
We use services to say that a 14.04 image is needed to run the “name”: “Ubuntu”

For multiple versions of the same image, add a another service.

So we say in our current 1.5 “plans = services”

We’ll name using the format “<order>_<cattype>_<name>”
...
We want to track the dynamic elements of a running application [virtualmachines, collaboration, customapp,

tag: flavor ,  name: DeployedFlavor - tell us the details of the flavour
tag: network_public_ipv4 ,  name: NetworkAttachPublicIPV4To - tell us the details of  private ipv4 *do the same for ipv6
tag: network_private_ipv4 ,  name: NetworkAttachPrivateIPV4To - tell us the details of public ipv4
tag: region ,  name: ClusterIAMin - tell us the cluster or grouping i am part of
tag:security, ssh, (or) userpasswd,  name: Identity - tell us the details of ssh or userpassw
tag: appsecurity, bitnami,  name: AppSecurity - tell us the details of initial application credentials  for logging in
tag: appaccess, bitnami,  name: AppAccess - tell us the details of accessing an application. This should be an URL based on routes resource
tag: apprepository, name:AppRepository - tells us the deployed url of the repository and its type (svn, git ..)
tag: apprepository, name:DatabaseOn - tells us the deployed database details (userid, password and url to access the db like psql://192.178..@root:pw
