...
Usecase1: Login
	create user using oc command and get token
	Fake Login using manual user sessions 
	set Api_key as use origin token 

Usecase2: ApiDispatcher 
  	Cut out megam_api use API Dispatcher pass through JSON
  	Form request url based on contorller GET/POST method request  
  	  - for example: marketplaces colletorller get
          -  https://192.168.0.8:8443/oapi/v1/templates 
          list of template passthrough jsons    
Usecase3: List Marketplaces 
  	manually create templates in default namespace
  	List marketplaces GET using bellow request and pares json show (changes in embber json parsing)
    	   - https://192.168.0.8:8443/oapi/v1/templates
  
Usecase4: Create and List secret
  
  Creates Generate ssh-pub and private keys and form as like below 
  Upload pub and private keys parse pub and private files form below json and create secrete
  List secrets GET using bellow request and pares json show (changes in embber json parsing) 
     https://192.168.1.8:8443/api/v1/namespaces/{namespace}/secrets
...
Usecase5: Step 2
    	List Templates as per usecase3. [sample template](https://github.com/vijaykanthm28/verticedev/tree/master/proposals/sample_vm_template.yaml)
    	User will select a template 
         
Usecase6: Step 3 
	List secrets as per usecase4
